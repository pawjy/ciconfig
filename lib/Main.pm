    ...
    set => sub {
      ...
      my $insert_step = sub {
        ...
        for my $rule (sort { $a->{name} cmp $b->{name} } @$rules) {
          ...
          $step->{depends_on} = ['build', map { @{$step_names->{$_} or []} } @{$args{prev_phases} or []}];
...

      {
        ...
        $rules = $insert_step->($rules, phase => 'deploy',
                                prev_phases => [qw(test)]);
        die if @$rules;
      }
...

      {
        ...
        $crules = $insert_step->($crules, phase => 'cleanup1',
                                 prev_phases => [qw(test deploy failed)],
                                 before_nested => 1);
