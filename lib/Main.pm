    ...
    set => sub {
      ...
      my $insert_step = sub {
        ...
        for my $rule (sort { $a->{name} cmp $b->{name} } @$rules) {
          ...
          $step->{when}->{status} = ['failure']
              if $args{failed};
          $step->{failure} = 'ignore' if $args{cleanup} or $args{failed};
      ...
      if ($with_artifacts) {
        push  @$init_commands, sub {
          my $name = shift;
          return () if $name =~ /^failed-/ or $name =~ /^cleanup-/;
          return (
...

      {
        ...
        $rules = $insert_step->($rules, test => 1);
        die if @$rules;
      }

      {
        my $rules = delete $json->{_deploy_rules} || [];
        $rules = $insert_step->($rules);
        die if @$rules;
      }
      
      {
        my $rules = delete $json->{_failed_rules} || [];
        $rules = $insert_step->($rules, failed => 1);
        die if @$rules;
      }

      {
        my $crules = delete $json->{_cleanup_rules} || [];
        $crules = $insert_step->($crules, cleanup => 1, before_nested => 1);
        
        my $cleanup_rules = $insert_step->($cleanup_rules, cleanup => 1);
        die if @$cleanup_rules;
        
        $crules = $insert_step->($crules, cleanup => 1);
        die if @$crules;
      }
    }, # set
  },
