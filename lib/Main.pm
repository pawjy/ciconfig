    ...
    set => sub {
      ...
      if ($with_artifacts) {
        ...
        push @$terminate_commands, sub {
          ...
          push @$aurls, $aurl;
          return (
            (sprintf 'aws s3 sync $CIRCLE_ARTIFACTS s3://%s/%s$DRONE_REPO/$DRONE_BUILD_NUMBER/%s && echo "Artifacts: <%s>"', $with_artifacts->{s3_bucket}, $with_artifacts->{s3_prefix}, $name, $aurl),
          );
        };
