          ...
          if (delete $input->{_with_macos}) {
            if ($input->{_with_macos_latest_perl}) {
              $json->{jobs}->{test}->{strategy}->{matrix}->{include} = [(map {
                {perl_version => $_, os => 'ubuntu-latest'};
              } @{$input->{_perl_versions}}), (map {
                {perl_version => $_, os => 'macos-latest'};
              } 'latest')];
            } else {
              $json->{jobs}->{test}->{strategy}->{matrix}->{include} = [(map {
                {perl_version => $_, os => 'ubuntu-latest'};
  ...
  droneci => {
    ...
    set => sub {
      ...
      my $insert_step = sub {
        my ($rules, %args) = @_;
        my $other_rules = {};
        for my $key (sort { $a cmp $b } keys %$rules) {
          my $rule = $rules->{$key};
          ...
          $step->{name} = $args{name}->($key);
          $step->{image} = 'quay.io/wakaba/docker-perl-app-base';
          $step->{commands} = [];
          $step->{when}->{status} = ['failure', 'success']
              if $args{cleanup};

          push @{$step->{commands}}, map { droneci_step $_ }
              @$init_commands, @{$rule->{commands}};
      ...
      if (my $dd = delete $json->{_docker}) {
        ...
        push @{$bstep->{commands}}, map { droneci_step $_ } @$init_commands;
        ...
        if ($dd->{with_nested}) {
          ...
          $cleanup_rules->{nested} = {commands => []};
          push @{$cleanup_rules->{nested}->{commands}},
      ...
      for (sort { $a cmp $b } keys %{$json->{_test_steps} or {}}) {
        my $tstep = {};
        ...
        push @{$tstep->{commands}}, map {
          droneci_step $_;
        } @$init_commands, @{$json->{_test_steps}->{$_}};
      ...
      for (sort { $a cmp $b } keys %{$json->{_deploy_steps} or {}}) {
        my $dstep = {};
        push @{$json->{steps} ||= []}, $dstep;
        $dstep->{name} = 'deploy-' . $_;
        $dstep->{image} = 'quay.io/wakaba/docker-perl-app-base';
        $dstep->{commands} = [];

        push @{$dstep->{commands}}, map {
          droneci_step $_;
        } @{$json->{_deploy_steps}->{$_}};
        push @{$dstep->{volumes} ||= []}, @$volumes if @$volumes;
      }
      delete $json->{_deploy_steps};

      {
        my $crules = delete $json->{_cleanup_rules};
        $crules = $insert_step->($crules, name => sub {
                                   return 'cleanup--' . $_[0];
                                 }, cleanup => 1, before_nested => 1);
        
        my $cleanup_rules = $insert_step->($cleanup_rules, name => sub {
                                             return 'cleanup-' . $_[0];
                                           }, cleanup => 1);
        die if keys %$cleanup_rules;
